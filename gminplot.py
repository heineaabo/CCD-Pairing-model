...
plt.title('Energy as a function of basis states',fontsize=20)
plt.xlabel('Number of basis states',fontsize=16)
plt.ylabel('Energy [Hartrees]',fontsize=16)
#plt.plot(x,E02,label=r'CCD $\alpha=0.2$')
plt.plot(x,E04,label='CCD')
#plt.plot(x,E06,label=r'CCD $\alpha=0.6$')
#plt.plot(x,E08,label=r'$\alpha=0.8$')
#plt.plot(x,E10,label=r'$\alpha=1.0$')
plt.plot(x,CI_E_g1,label='CI')
plt.legend()
if save:
    plt.savefig('figures/energy_gmin1_p2.pdf')

# ------- Energy Difference ------- #
plt.figure(figsize=[7,5])
plt.title('|$E_{CI} - E_{CCD}$|',fontsize=20)
plt.xlabel('Number of basis states',fontsize=16)
plt.ylabel(r'$\Delta E$',fontsize=16)
#plt.plot(x,np.abs(E02-CI_E_g1),label=r'CCD $\alpha=0.2$')
plt.plot(x,np.abs(E04-CI_E_g1))
#plt.plot(x,np.abs(E06-CI_E_g1),label=r'CCD $\alpha=0.6$')
#plt.plot(x,np.abs(E08-CI_E_g1),label=r'CCD $\alpha=0.8$')
#plt.plot(x,np.abs(E10-CI_E_g1),label=r'CCD $\alpha=1.0$')
#plt.legend()
if save:
    plt.savefig('figures/Ediff_gmin1_p2.pdf')
...
plt.legend()
if save:
    plt.savefig('figures/conv_gmin1_p2.pdf')
...
plt.legend()
if save:
    plt.savefig('figures/time_gmin1_p2.pdf')
